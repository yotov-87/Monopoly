...
public class GameController : ControllerBase
{
    ...
    [HttpPost("{gameId}/build-house/{cellId}")]
    [Authorize]
    public async Task<IActionResult> BuildHouse(int gameId, int cellId)
    {
        // Get user ID from JWT token
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier);
        if (userIdClaim == null || !int.TryParse(userIdClaim.Value, out int userId))
        ...
        try
        {
            var playgroundInfo = await _gameService.BuildHouseAsync(gameId, userId, cellId);
    ...
    [HttpPost("{gameId}/build-hotel/{cellId}")]
    [Authorize]
    public async Task<IActionResult> BuildHotel(int gameId, int cellId)
    {
        // Get user ID from JWT token
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier);
        if (userIdClaim == null || !int.TryParse(userIdClaim.Value, out int userId))
        ...
        try
        {
            var playgroundInfo = await _gameService.BuildHotelAsync(gameId, userId, cellId);
